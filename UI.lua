...
local frame, content, contentHeight
...
local function resolveAssignment(a)
    ...
    if a.kind == "equip" then
        ...
        if name then return "Equip: " .. name, icon end
        return name, icon
    end
    if a.kind == "item" then
...
local function build()
    ...
    -- SetResizeBounds exists on modern clients; SetMinResize/SetMaxResize on older.
    ...
    pushBtn:SetScript("OnClick", function()
        local ok, err = CRP.Comms:PushPull()
...
local function makeUpcomingRow(parent)
    ...
    row.bg = bg
    row:SetScript("OnEnter", function(s) s.bg:SetColorTexture(1, 1, 1, 0.08) end)
...
local function updateUpcoming(curIdx, pulls, my)
    ...
    for offset = 1, UPCOMING_COUNT do
        ...
        row:ClearAllPoints()
        row:SetPoint("TOPLEFT", upcomingContainer, "TOPLEFT", 0,
            -UPCOMING_HEADER_H - (offset - 1) * UPCOMING_ROW_H)
        row:SetPoint("TOPRIGHT", upcomingContainer, "TOPRIGHT", 0,
            -UPCOMING_HEADER_H - (offset - 1) * UPCOMING_ROW_H)
        ...
        if not pull then
            ...
            for _, a in ipairs(pull.assignments or {}) do
                if hasContent(a) and (not my or isMyAssignment(a)) then
                    rel[#rel + 1] = a
                    if #rel >= UPCOMING_MAX_ICONS then break end
                end
            end
            ...
            for i = #rel + 1, #row.icons do row.icons[i]:Hide() end
            ...
            if my then
                ...
                row:SetScript("OnClick", function() CRP.Plan:SetCurrentPullIdx(upIdx) end)
            end
            row:Show()
        end
    end
...
function Window:ApplyMode()
    ...
    if my then
        bossLine:Hide(); packLine:Hide(); progressHeader:Hide()
        for _, line in ipairs(progressLines) do line:Hide() end
    else
        bossLine:Show(); packLine:Show(); progressHeader:Show()
    end
end
...
function Window:Refresh()
    ...
    if not plan or #pulls == 0 then
        ...
        packLine:SetText("")
        noteBox:SetText("")
        for _, row in ipairs(assignRowPool) do row:Hide() end
        for _, row in ipairs(upcomingRows) do row:Hide() end
        if upcomingContainer then upcomingContainer:Hide() end
        content:SetHeight(1)
        emptyMsg:Show()
        return
    end
    ...
    if not my then
        if pull.bossId then
            local boss = CRP.Plan:BossById(pull.bossId)
            bossLine:SetText("Boss: " .. ((boss and boss.name) or pull.bossId))
        else
            bossLine:SetText("")
        end
        ...
        for i = #entries + 1, #progressLines do progressLines[i]:Hide() end
    ...
    contentHeight = math.max(1, #shown * ROW_H)
...
-- Simple import dialog: big edit box + Import button.
-- Scrollable jump-to-pull popup.
local pullPopup, pullPopupRowPool, pullPopupContent

function Window:TogglePullPopup(anchorBtn)
    if pullPopup and pullPopup:IsShown() then
        pullPopup:Hide()
        return
    end
    if not pullPopup then
        ...
        pullPopup:SetSize(260, 260)
        pullPopup:EnableMouse(true)
        pullPopup:Hide()
        if pullPopup.SetBackdrop then
        ...
        local scroll = CreateFrame("ScrollFrame", "CafeRaidPlannerPullPopupScroll",
            pullPopup, "UIPanelScrollFrameTemplate")
        scroll:SetPoint("TOPLEFT", 8, -8)
        scroll:SetPoint("BOTTOMRIGHT", -28, 8)
        pullPopupContent = CreateFrame("Frame", nil, scroll)
        pullPopupContent:SetSize(1, 1)
        scroll:SetScrollChild(pullPopupContent)
        pullPopupRowPool = {}

        -- Close-on-outside-click: watch mouse-down on UIParent.
        pullPopup:SetScript("OnShow", function()
            pullPopup._closer = pullPopup._closer or CreateFrame("Frame", nil, UIParent)
            pullPopup._closer:SetAllPoints(UIParent)
            pullPopup._closer:SetFrameStrata("FULLSCREEN_DIALOG")
            pullPopup._closer:EnableMouse(true)
            pullPopup._closer:SetScript("OnMouseDown", function(_, button)
                if button then pullPopup:Hide() end
            end)
            pullPopup._closer:Show()
            pullPopup:SetFrameLevel((pullPopup._closer:GetFrameLevel() or 0) + 10)
        end)
        pullPopup:SetScript("OnHide", function()
            if pullPopup._closer then pullPopup._closer:Hide() end
        end)
    end
    ...
    for i, pull in ipairs(pulls) do
        ...
        row:ClearAllPoints()
        row:SetPoint("TOPLEFT", pullPopupContent, "TOPLEFT", 0, -(i - 1) * ROW)
    ...
    -- ScrollChild needs explicit dimensions — with width 1, the rows'
    -- RIGHT anchor collapses to 1px and the buttons look empty/dead.
    local scroll = _G["CafeRaidPlannerPullPopupScroll"]
    pullPopupContent:SetWidth(scroll and scroll:GetWidth() or 220)
    pullPopupContent:SetHeight(math.max(1, #pulls * ROW))
...
function Window:ShowImport()
    if not importDialog then
        ...
        if importDialog.SetBackdrop then
        ...
        local scroll = CreateFrame("ScrollFrame", "CafeRaidPlannerImportScroll",
            importDialog, "InputScrollFrameTemplate")
        ...
        importBtn:SetScript("OnClick", function()
            ...
            importDialog:Hide()
            Window:Refresh()
            print("|cff38c24fCafeRaidPlanner:|r imported plan with "
                .. #(env.preset.pulls or {}) .. " pulls, "
                .. #(env.packs or {}) .. " packs.")
        end)
